...
void yolov3(int model_height, int model_width, string source_type, const int BatchSize, vector<string>& params, string config_file)
{
	...
	if (config_file.empty()) {
		cfgname = "config/yolov3-tiny_" + to_string(model_height) + "x" + to_string(model_width) + "_" + source_type + "_b" + to_string(BatchSize) + ".cfg";
	}
	else {
		cfgname = trim(config_file);
	}
...
int main(int argc, char** argv)
{
	...
	string config_file = "configs/yolov3-tiny.cfg";
